...
public class HardcoreDarkness
{
	...
	{
		event.registerServerCommand(new CommandHardcoreDarkness());
	}
	...
	public void clientTick(TickEvent.ClientTickEvent event)
	{
		if (event.phase == Phase.END)
		{
				...
				for (PotionEffect potionEffect : effects) {						
					...
					if(potionEffect.getEffectName().equals("effect.nightVision")) {
						pass = false;
						lightingRefreshScheduled = false;
						
						//player.sendMessage(new TextComponentTranslation("true"));
						break;
					}
				}
					 */
